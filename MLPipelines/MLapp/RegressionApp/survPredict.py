...
def predict(mystr):
    try:
        ...
        model = load('ClassificationModel.sav')
        prediction = model.predict(numList)
        print("Classification: Correctly formatted Data was entered!\n")
        if prediction[0] == 1:
            return "Yes"
        else:
            return "No"
    except:
        print("Classification: Incorrectly formatted Data was entered!\n")
